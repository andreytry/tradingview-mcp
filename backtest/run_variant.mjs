...
for(const [sym,tick,pv] of SYMS){
  ...
  for(const [sec,tf] of [[300,0],[900,1],[3600,2]]){
    ...
    zs=zs.concat(detect(h,tf,(n)=>idxAtTime(EARLY? h[n].time : (n+1<h.length?h[n+1].time:h[n].time+sec))));
  }
...
writeFileSync(`${D}/trades_6m${process.env.EARLY==='1'?'_early':''}.json`,JSON.stringify(all,null,1));
